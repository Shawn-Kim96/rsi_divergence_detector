    ...
    def plot_divergence(df, divergence_df, title='', save_fig=False):
        ...
        for idx, row in divergence_df.iterrows():
            try:
                ...
                entry_price = df.loc[entry_datetime, price_name]
        ...
        # Optional: Add TP and SL annotations
        # for idx, row in divergence_df.iterrows():
        #     try:
        #         tp = row['TP']
        #         sl = row['SL']
        #         divergence_type = row['divergence']
        #         color = 'green' if divergence_type == 'Bullish Divergence' else 'red'

        #         # Add TP and SL annotations
        #         axes[0].text(df.index[-1], tp, f'TP: {tp:.2f}', color='green', fontsize=10, verticalalignment='bottom')
        #         axes[0].text(df.index[-1], sl, f'SL: {sl:.2f}', color='red', fontsize=10, verticalalignment='top')
